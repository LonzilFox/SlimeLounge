...
export const DEFAULT_GAME_CHIP_RULES={
  ...
  poker:{smallBlind:10,bigBlind:20,entryFee:10,aiEntryFee:4},
...
export function normalizeEconomy(raw={}){
  ...
  const pk=src.poker||{},sb=boundedInt(pk.smallBlind,1,100000,DEFAULT_GAME_CHIP_RULES.poker.smallBlind),bb=boundedInt(pk.bigBlind,sb,1000000,DEFAULT_GAME_CHIP_RULES.poker.bigBlind);
  out.gameRules.poker={smallBlind:sb,bigBlind:bb,entryFee:fee(src,'poker'),aiEntryFee:fee(src,'poker','aiEntryFee')};
...
export function migrateEconomyV029(raw={}){
  ...
  for(const [kind,rec] of Object.entries(DEFAULT_GAME_CHIP_RULES)){
    ...
    for(const key of ['stake','minBet','maxBet','defaultBet','smallBlind','bigBlind','rankStep','pointsPerChip','forfeitPenalty'])if(rec[key]!=null&&(r[key]==null||Number(r[key])===Number(old[key])))r[key]=rec[key];
  }
