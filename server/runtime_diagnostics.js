export function createRuntimeDiagnostics({requestIpInfo,version='0.0.0',trustProxy=false}){
  const isExternalWait=path=>['/api/music/probe','/api/music/fallback','/api/music/search','/api/music/resolve'].includes(path);
  ...
  function bucket(now=Date.now()){
    ...
    if(!b||b.at!==key){b={at:key,requests:0,status2xx:0,status4xx:0,status5xx:0,rateLimited:0,slow:0,externalSlow:0,totalMs:0,maxMs:0,paths:{}};buckets.push(b);if(buckets.length>10)buckets.splice(0,buckets.length-10)}
    return b;
  }
  ...
  function beginRequest(req,pathname){
    ...
    return {finish(status=200){
      const ms=Date.now()-started,b=bucket(),meta=metaFor(req);b.requests++;b.totalMs+=ms;b.maxMs=Math.max(b.maxMs,ms);if(status>=500)b.status5xx++;else if(status>=400)b.status4xx++;else if(status>=200)b.status2xx++;if(status===429)b.rateLimited++;const intentionalWait=pathname==='/api/room/poll',externalWait=isExternalWait(pathname);if(ms>=1000&&externalWait)b.externalSlow++;else if(ms>=1000&&!intentionalWait)b.slow++;b.paths[pathname]=(b.paths[pathname]||0)+1;
      ...
      const noteworthy=status>=500||status===429||(ms>=1000&&pathname!=='/api/room/poll'&&!isExternalWait(pathname));
  ...
  function selectedUserLog(userId=''){
    ...
    return {userId,requests,events,stats:{requests5m:last5.length,rateLimited5m:last5.filter(x=>x.status===429).length,errors5m:last5.filter(x=>x.status>=400).length,slow5m:last5.filter(x=>x.ms>=1000&&x.path!=='/api/room/poll'&&!isExternalWait(x.path)).length,externalSlow5m:last5.filter(x=>x.ms>=1000&&isExternalWait(x.path)).length,avgRttMs:avg,p95RttMs:p95,maxRttMs:rtts.at(-1)||0,rttSamples:rtts.length,topPaths:Object.entries(pathTotals).sort((a,b)=>b[1]-a[1]).slice(0,8).map(([path,count])=>({path,count}))}};
  }
  function summary(extra={},filterUserId=''){
    const now=Date.now();prune(now);const last=traffic.filter(x=>now-x.at<60000),reqs=last.length,r429=last.filter(x=>x.status===429).length,e4=last.filter(x=>x.status>=400&&x.status<500).length,e5=last.filter(x=>x.status>=500).length,slow=last.filter(x=>x.ms>=1000&&x.path!=='/api/room/poll'&&!isExternalWait(x.path)).length,externalSlow=last.filter(x=>x.ms>=1000&&isExternalWait(x.path)).length,serverLast=last.filter(x=>x.path!=='/api/room/poll'&&!isExternalWait(x.path)),avgMs=serverLast.length?Math.round(serverLast.reduce((a,x)=>a+x.ms,0)/serverLast.length):0,maxMs=serverLast.length?Math.max(...serverLast.map(x=>x.ms)):0;const pathTotals={};for(const x of last)pathTotals[x.path]=(pathTotals[x.path]||0)+1;const topPaths=Object.entries(pathTotals).sort((a,b)=>b[1]-a[1]).slice(0,10).map(([path,count])=>({path,count}));const m=process.memoryUsage();
    return {ok:true,version,startedAt,uptimeSec:Math.floor(process.uptime()),trustProxy,eventLoopLagMs,maxEventLoopLagMs,memory:{rssMB:+(m.rss/1048576).toFixed(1),heapUsedMB:+(m.heapUsed/1048576).toFixed(1),heapTotalMB:+(m.heapTotal/1048576).toFixed(1)},lastMinute:{windowSec:60,requests:reqs,rateLimited:r429,status4xx:e4,status5xx:e5,slow,externalSlow,avgMs,maxMs,topPaths},websocket:{...ws},recent:recent.slice(-60).reverse(),selectedUser:selectedUserLog(filterUserId),...extra}
  }
