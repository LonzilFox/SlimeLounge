...
const pkg=JSON.parse(fs.readFileSync(path.join(root,'package.json'),'utf8'));if(pkg.version!=='0.3.9'){console.error('[FAIL] version',pkg.version);fail=1}else console.log('[OK] version 0.3.9');
const workerSrc=fs.readFileSync(path.join(root,'src/index.js'),'utf8');if(!workerSrc.includes("version:'0.3.9'")){console.error('[FAIL] worker health version');fail=1}else console.log('[OK] worker health version 0.3.9');
