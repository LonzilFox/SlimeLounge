...
const required=['package.json','wrangler.jsonc','local_server.js','server/music_service.js','server/economy.js','server/rankings.js','server/http_security.js','server/runtime_diagnostics.js','server/network_sync.js','server/static_service.js','server/game_lifecycle.js','server/ws_transport.js','server/room_change_hub.js','server/action_receipts.js','server/ws_session_auth.js','server/http_room_transport.js','server/device_identity.js','server/device_profile.js','server/user_merge.js','server/release_notes.js','server/voice_config.js','server/progression.js','public/index.html','public/styles.css','public/styles-games.css','public/styles-responsive.css','public/origin-migrate.html','public/origin-migrate.js','public/app.js','public/music-ui.js','public/chat-ui.js','public/game-dice.js','public/game-poker.js','public/game-mahjong.js','public/game-doudizhu.js','public/progression-ui.js','public/admin-economy.js','public/app-games.js','src/index.js','shared/games.js','shared/dice.js','shared/extra_games.js','shared/riichi.js','shared/doudizhu.js','tools/01_runtime/02_owner_recovery.sh'];
...
if(!serverSrc.includes('function userIsOnline(userId)')||!serverSrc.includes("process.platform==='linux'&&!process.env.DISPLAY&&!process.env.WAYLAND_DISPLAY")){console.error('[FAIL] Linux/tab server compatibility');fail=1}else console.log('[OK] Linux/tab server compatibility');
...
const appUi=fs.readFileSync(path.join(root,'public/app.js'),'utf8')+fs.readFileSync(path.join(root,'public/music-ui.js'),'utf8')+fs.readFileSync(path.join(root,'public/chat-ui.js'),'utf8')+fs.readFileSync(path.join(root,'public/game-dice.js'),'utf8')+fs.readFileSync(path.join(root,'public/game-poker.js'),'utf8')+fs.readFileSync(path.join(root,'public/game-mahjong.js'),'utf8')+fs.readFileSync(path.join(root,'public/game-doudizhu.js'),'utf8')+fs.readFileSync(path.join(root,'public/progression-ui.js'),'utf8')+fs.readFileSync(path.join(root,'public/admin-economy.js'),'utf8')+fs.readFileSync(path.join(root,'public/app-games.js'),'utf8'),cssFiles=['public/styles.css','public/styles-games.css','public/styles-responsive.css'],cssUi=cssFiles.map(f=>fs.readFileSync(path.join(root,f),'utf8')).join('\n');
...
const v17Checks=[
  ...
  ['admin three tabs',appUi.includes('用户与设备')&&appUi.includes('工号审核')&&appUi.includes('排行榜校验')],
...
const v18Checks=[
  ...
  ['chip admin before rankings',appUi.includes('data-admin-tab="chips"')&&appUi.indexOf('data-admin-tab="chips"')<appUi.indexOf('data-admin-tab="rankings"')&&serverSrc.includes('/api/admin/chips')&&appUi.includes('chipRulesSave')],
...
const v23Checks=[
  ...
  ['chat voice channels',ROOM_DEFS.filter(r=>r.category==='chat'&&r.channelType==='voice'&&r.voiceAllowed).length===2&&serverMainSrc.includes("def.voiceAllowed")&&workerSrc.includes("def.voiceAllowed")],
  ...
  ['daily chip checkin',serverMainSrc.includes("url.pathname==='/api/wallet/checkin'")&&workerSrc.includes("url.pathname==='/wallet/checkin'")&&appUi.includes('dailyChipReward||1000')],
