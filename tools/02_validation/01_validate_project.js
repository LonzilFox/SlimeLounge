...
const required=['package.json','local_server.js','server/chat_state.js','server/release_rewards.js','server/music_service.js','server/economy.js','server/rankings.js','server/http_security.js','server/runtime_diagnostics.js','server/network_sync.js','server/static_service.js','server/game_lifecycle.js','server/ws_transport.js','server/room_change_hub.js','server/action_receipts.js','server/ws_session_auth.js','server/http_room_transport.js','server/device_identity.js','server/device_profile.js','server/user_merge.js','server/release_notes.js','server/voice_config.js','server/progression.js','server/leisure_service.js','server/single_game_service.js','server/input_validation.js','server/music_http_routes.js','server/open_browser.js','public/index.html','public/styles.css','public/styles-games.css','public/styles-responsive.css','public/ui-overrides.css','public/origin-migrate.html','public/origin-migrate.js','public/app.js','public/ui-enhancements.js','public/music-ui.js','public/chat-ui.js','public/game-dice.js','public/game-poker.js','public/game-mahjong.js','public/game-doudizhu.js','public/progression-ui.js','public/leisure-ui.js','public/admin-economy.js','public/app-games.js','shared/games.js','shared/dice.js','shared/extra_games.js','shared/riichi.js','shared/doudizhu.js','tools/01_runtime/02_owner_recovery.sh'];
...
const counts={};for(const r of ROOM_DEFS)if(r.game)counts[r.game]=(counts[r.game]||0)+1;const expect={dice:5,gomoku:5,xiangqi:3,chess:3,blackjack:3,poker:2,sudoku:3,minesweeper:5,go:3,mahjong:2,uno:2,doudizhu:3,tetris:1};for(const [k,v] of Object.entries(expect)){if(counts[k]!==v){console.error('[FAIL]',k,counts[k],v);fail=1}else console.log('[OK]',k,v)}
...
const appUi=fs.readFileSync(path.join(root,'public/app.js'),'utf8')+fs.readFileSync(path.join(root,'public/music-ui.js'),'utf8')+fs.readFileSync(path.join(root,'public/chat-ui.js'),'utf8')+fs.readFileSync(path.join(root,'public/game-dice.js'),'utf8')+fs.readFileSync(path.join(root,'public/game-poker.js'),'utf8')+fs.readFileSync(path.join(root,'public/game-mahjong.js'),'utf8')+fs.readFileSync(path.join(root,'public/game-doudizhu.js'),'utf8')+fs.readFileSync(path.join(root,'public/progression-ui.js'),'utf8')+fs.readFileSync(path.join(root,'public/leisure-ui.js'),'utf8')+fs.readFileSync(path.join(root,'public/admin-economy.js'),'utf8')+fs.readFileSync(path.join(root,'public/app-games.js'),'utf8'),cssFiles=['public/styles.css','public/styles-games.css','public/styles-responsive.css','public/ui-overrides.css'],cssUi=cssFiles.map(f=>fs.readFileSync(path.join(root,f),'utf8')).join('\n');
...
const v22Checks=[
  ...
  ['away/offline newest-first sorting',appUi.includes("if(ra>=7)return (Number(b?.lastSeenAt)||0)-(Number(a?.lastSeenAt)||0)")&&serverMainSrc.includes("if(px>=5)return (y.lastSeenAt||0)-(x.lastSeenAt||0)")],
];
...
const v26Checks=[
  ...
  ['hostname limitation stays in admin diagnostics',appUi.includes('浏览器无法读取 Windows 的真实电脑主机名')]
];
...
const v28Checks=[
 ...
 ['dynamic economy / entry fees',adminEconomySrc.includes('econDaily')&&adminEconomySrc.includes('fee_${kind}')&&adminEconomySrc.includes('aifee_${kind}')&&serverMainSrc.includes('chargeRoundEntryFee')&&singleGameSrc.includes('/api/game/entry')],
...
const v31Checks=[
 ...
 ['away/offline separate sort',serverMainSrc.includes("u.online&&u.presenceStatus==='away'?6:7")&&appUi.includes("?'暂离':'离线'")&&appUi.includes('暂离仅表示页面切到后台')],
