...
const input=read('server/input_validation.js'),http=read('server/http_security.js'),local=read('local_server.js'),leisure=read('server/leisure_service.js'),leisureUi=read('public/leisure-ui.js'),games=read('shared/games.js'),gameUi=read('public/app-games.js'),ui=read('public/ui-v038.js'),css=read('public/styles-v038.css');
...
const old={version:1,fishing:{...DEFAULT_LEISURE_CONFIG.fishing,fish:DEFAULT_LEISURE_CONFIG.fishing.fish.map(x=>({...x,basePrice:x.id==='sardine'?54321:(x.id==='anchovy'?95:x.basePrice)}))},market:{...DEFAULT_LEISURE_CONFIG.market,tickSeconds:45}};
const migrated=normalizeLeisureConfig(old);
ok(migrated.version===2&&migrated.fishing.fish.find(x=>x.id==='sardine')?.basePrice===54321,'custom fish price overwritten');
ok(migrated.fishing.fish.find(x=>x.id==='anchovy')?.basePrice===90,'untouched old fish price was not softly migrated');
