...
try{
  ...
  for(let guard=0;guard<12;guard++){s=await snap(ac);if(s.game.phase==='result')break;const i=s.game.turnSeat,p=s.game.seats?.[i];if(!p)throw Error('blackjack turn seat missing');const c=p.userId===a.userId?ac:p.userId===b.userId?bc:null;if(!c)throw Error('unexpected non-human on human table');await room(c,{type:'stand'});}s=await snap(ac);if(s.game.phase!=='result')throw Error('human blackjack hand did not finish');
  ...
  for(let guard=0;guard<80;guard++){await sleep(80);s=await snap(ac);if(s.game.phase==='result')break;if(s.game.turnSeat!=null&&s.game.seats[s.game.turnSeat]?.userId===a.userId)await room(ac,{type:'stand'});}s=await snap(ac);if(s.game.phase!=='result')throw Error('AI practice blackjack hand did not finish');const afterAi=await post('/api/auth',ac);if(afterAi.profile.chips!==walletBeforeAi-3)throw Error(`AI practice should only charge the per-hand AI fee: ${walletBeforeAi} -> ${afterAi.profile.chips}`);
