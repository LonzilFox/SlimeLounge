...
try{
  const health=await wait();ok(health.version==='0.4.8'&&!('port' in health)&&!('trustProxy' in health),'health data minimization/version failed');
  ...
  console.log('[OK] v0.4.8 security boundaries / authoritative fishing / Blackjack rules+RNG / market countdown / admin rendering');
