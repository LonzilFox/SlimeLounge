...
ok(DEFAULT_LEISURE_CONFIG.version===9&&DEFAULT_PROGRESSION_CONFIG.version===13,'current leisure/progression schema mismatch');
...
ok(read('public/accessories/halo.svg').includes('hollow angel halo')&&ui.includes("'/accessories/halo.svg'")&&ui.includes("viewBox:'0 0 60 24'")&&ui.includes("M14 3h32v3H14z"),'angel halo is not a hollow segmented pixel ring in file/runtime renderer');
...
ok(progression.includes('version:13')&&progression.includes('migrateAccessoryV12')&&progression.includes("name:'天使光环'")&&progression.includes("slot:'head'"),'halo head-slot semantic migration missing');
...
ok(pc.version===13&&haloAcc?.slot==='head'&&haloAcc.name==='天使光环','halo did not migrate to head/angel semantics');
...
for(const f of ['fireflies','moon-motes','petals','aurora-dust','nebula-dust'])ok(fs.existsSync(path.join(root,`public/accessories/${f}.svg`)),`new particle asset missing: ${f}`);
for(const id of ['acc_fireflies','acc_moon_motes','acc_petals','acc_aurora_dust','acc_nebula_dust'])ok(DEFAULT_PROGRESSION_CONFIG.shop.accessories.some(x=>x.id===id),`particle product missing: ${id}`);
...
const n46=notes.find(x=>x.id==='v0.4.6'),n47=notes.find(x=>x.id==='v0.4.7');ok(n46?.items?.length>=8&&n47?.items?.length>=13&&n47.date==='2026-09-04','v0.4.6/v0.4.7 release notes are incomplete');
