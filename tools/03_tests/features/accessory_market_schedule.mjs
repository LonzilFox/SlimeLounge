...
ok(DEFAULT_LEISURE_CONFIG.version===8&&DEFAULT_PROGRESSION_CONFIG.version===12,'current leisure/progression schema mismatch');
ok(html.includes('/icons/fishing-nav.svg?v=0.4.7')&&!html.includes('<span class="nav-icon">🎣</span><span class="nav-label">钓鱼</span>'),'pixel fishing nav icon missing');
...
ok(read('public/accessories/halo.svg').includes('fill-rule="evenodd"')&&ui.includes("'/accessories/halo.svg'")&&ui.includes("viewBox:'0 0 56 22'"),'angel halo is not a hollow pixel ring in file/runtime renderer');
...
ok(progression.includes('version:12')&&progression.includes('migrateAccessoryV12')&&progression.includes("name:'天使光环'")&&progression.includes("slot:'head'"),'halo head-slot semantic migration missing');
...
ok(pc.version===12&&haloAcc?.slot==='head'&&haloAcc.name==='天使光环','halo did not migrate to head/angel semantics');
...
const lc=normalizeLeisureConfig({version:7,market:{minBaseRatio:0.1,maxBaseRatio:10,sessionStartHour:8,sessionEndHour:21,weekendClosed:true,sessionTimeZone:'Asia/Shanghai'},fishing:{rods:[{id:'rod_training',name:'自定义训练竿',price:777,enabled:true}],fish:[{id:'carp',name:'自定义鲤鱼',basePrice:125,difficulty:15,rarity:1,minSize:30,maxSize:78,enabled:true}]}});
...
ok(lc.market.noiseScale===0.045&&lc.market.eventChance===0.009&&lc.market.eventMinVolScale===0.025&&lc.market.eventExtraVolScale===0.05,'random-vs-trend tuning is not the v0.4.7 balance');
ok(lc.fishing.rods[0].price===777&&lc.fishing.fish[0].basePrice===125,'v0.4.7 migration overwrote existing rod/fish price');
ok(leisure.includes('outerMin*(1+buffer)')&&leisure.includes('outerMax*(1-buffer)')&&leisure.includes('edgeGuardStrength'),'market does not approach limits with inward buffer/edge guard');
...
const n46=notes.find(x=>x.id==='v0.4.6'),n47=notes.find(x=>x.id==='v0.4.7');ok(n46?.items?.length>=8&&n47?.items?.length>=9,'v0.4.6/v0.4.7 release notes are incomplete');
