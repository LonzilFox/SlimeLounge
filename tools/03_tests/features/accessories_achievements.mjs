...
ok(html.includes('ui-enhancements.js?v=0.4.8&build=048')&&acc.includes('accessoryPalette')&&acc.includes('accessory-svg')&&!acc.includes('accessory-base-layer'),'single-SVG accessory renderer not loaded');
ok(prog.includes('ACCESSORY_ORIGINAL_COLORS')&&prog.includes("c.toLowerCase()==='#ffffff'")&&prog.includes('version:14'),'accessory original-color migration missing');
ok(read('public/accessories/star.svg').includes('viewBox="0 0 34 32"')&&read('public/accessories/star.svg').includes('shape-rendering="crispEdges"'),'star asset is not the v0.4.8 five-point pixel redesign');
...
ok(migrated.version===14&&migrated.shop.accessories.find(x=>x.id==='acc_bow')?.color==='#ff7fa2','white accessory migration did not recover bow original color');
...
console.log('[OK] v0.4.8 newest-chat paging / full admin expansion / market UI / single-SVG accessory recoloring+order / pet mood penalties');
