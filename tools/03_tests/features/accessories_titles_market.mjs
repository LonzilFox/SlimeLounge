...
try{
 ...
 const pr=await post('/api/progression/self',cred);ok(pr.progression.achievements.some(x=>x.id==='checkin_streak_30')&&pr.progression.shop.accessories.some(x=>x.id==='acc_sparkles')&&pr.progression.shop.titles.some(x=>x.id==='title_table_master'),'v0.4.7 shop/title/achievement payload missing');
 console.log('[OK] v0.4.7 single rankings / rods+quality fishing / titles+achievements / independent positions / dynamic OHLC market');
