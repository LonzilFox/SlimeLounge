...
ok(migrated.version===14&&migrated.account.levelXpBase===987&&migrated.pet.feedXp===77,'custom growth values were overwritten');
...
ok(leisureUi.includes('fish-track')&&leisureUi.includes('market-chart')&&leisureUi.includes('完全模拟')&&html.includes('leisure-ui.js?v=0.4.8&build=048'),'fishing/market client UI missing');
...
try{
  const health=await wait();ok(health.version==='0.4.8','server version mismatch');
  ...
  let st=await post('/api/leisure/state',cred);ok(st.fishing?.config?.fish?.length&&st.market?.config?.assets?.length,'leisure state unavailable');
  ...
  // v0.4.8: the market has a server-side short cooldown; legitimate back-to-back trades must respect it.
  ...
  console.log('[OK] v0.4.8 poker boss / formal blackjack / server-authoritative fishing / simulated market / patch-merge config / accessory toggle');
