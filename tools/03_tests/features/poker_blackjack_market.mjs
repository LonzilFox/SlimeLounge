...
ok(leisureUi.includes('fish-track')&&leisureUi.includes('market-chart')&&leisureUi.includes('完全模拟')&&html.includes('leisure-ui.js?v=0.4.7&build=047'),'fishing/market client UI missing');
...
try{
  const health=await wait();ok(health.version==='0.4.7','server version mismatch');
  ...
  // v0.4.7: the market has a server-side short cooldown; legitimate back-to-back trades must respect it.
  ...
  console.log('[OK] v0.4.7 poker boss / formal blackjack / server-authoritative fishing / simulated market / patch-merge config / accessory toggle');
