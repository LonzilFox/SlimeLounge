...
ok(leisure.includes('heartbeatMs:650')&&leisure.includes('version:5'),'fishing request-frequency migration missing');
...
ok(regression.includes("readFileSync(path.join(root,'package.json')")&&!regression.includes("health.version!=='0.4.5'"),'legacy regression still hardcodes release version');
ok(runner.includes('73_features_v043.mjs')&&runner.includes('74_features_v044.mjs'),'v0.4.3/v0.4.5 tests missing from npm run check');
...
try{
 ...
 console.log('[OK] v0.4.5 fish lock / diagnostics / pet penalty / responsive UI / limit+margin trading');
