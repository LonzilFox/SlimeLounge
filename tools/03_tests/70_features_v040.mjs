...
ok(app.includes("RANK_NAMES={chips:'筹码榜',market:'交易盈亏',fishing:'钓鱼盈利',dice:'摇骰子'") ,'market rank order missing');
...
ok(games.includes("isMarket=state.rankTab==='market'")&&games.includes('已实现盈亏')&&games.includes("post('/api/leaderboards',creds()"),'market leaderboard/authenticated client load missing');
...
ok(css.includes('.market-summary')&&css.includes('.growth-rank-grid')&&css.includes('.market-admin-holding')&&css.includes('.page:not(.hidden)'),'v0.4.4 layout consistency styles missing');
...
try{
  const health=await wait();ok(health.version==='0.4.4','wrong live version');
  ...
  console.log('[OK] v0.4.4 market admin / XP rankings / navigation lifecycle / leaderboard security / trade rate limit / static path boundary');
