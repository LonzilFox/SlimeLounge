...
ok(leisure.includes('function baseTickTrend')&&leisure.includes('(1-ratio)/.5')&&leisure.includes('(ratio-1)/1')&&leisure.includes("event=Math.random()<.018?(Math.random()<.5?-1:1)"),'symmetric base-ratio tick trend / market event logic missing');
...
ok(progression.includes("version:9")&&progression.includes('title_market_operator')&&progression.includes('title_lake_legend')&&progression.includes('checkin_streak_30')&&progression.includes('market_batch_10000'),'advanced titles / achievements / v9 migration missing');
...
ok(css.includes('.fishing-rod-grid')&&css.includes('.fish-card .fish-card-main>*')&&css.includes('.market-candle.up')&&css.includes('repeat(auto-fit'),'v0.4.5 responsive fishing/market styles missing');
...
try{
 ...
 const pr=await post('/api/progression/self',cred);ok(pr.progression.achievements.some(x=>x.id==='checkin_streak_30')&&pr.progression.shop.accessories.some(x=>x.id==='acc_sparkles')&&pr.progression.shop.titles.some(x=>x.id==='title_table_master'),'v0.4.5 shop/title/achievement payload missing');
 console.log('[OK] v0.4.5 single rankings / rods+quality fishing / titles+achievements / independent positions / dynamic OHLC market');
