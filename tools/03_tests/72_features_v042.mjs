...
ok(css.includes('.admin-tabs-v038{display:flex')&&css.includes('#adminView .admin-tabs-v038{position:sticky!important'),'admin top tabs / v0.4.4 sticky repair missing');
...
ok(migrated.version===8&&migrated.shop.accessories[0].color==='#ff7fa2','legacy white accessory color was not migrated back to its original main color');
...
ok(pkg.version==='0.4.4'&&!pkg.scripts?.deploy&&!pkg.devDependencies?.wrangler,'obsolete Cloudflare deployment entry still present');
...
console.log('[OK] v0.4.4 fishing resilience / sorting+bulk sell / catalogue / admin tabs / project cleanup');
