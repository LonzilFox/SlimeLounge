...
ok(migrated.version===7&&migrated.fishing.fish.find(x=>x.id==='sardine')?.basePrice===1775,'custom fish price overwritten');
...
try{
  const health=await wait();ok(health.version==='0.4.6'&&!('port' in health)&&!('trustProxy' in health),'health data minimization/version failed');
  ...
  console.log('[OK] v0.4.6 security boundaries / authoritative fishing / Blackjack rules+RNG / market countdown / admin rendering');
