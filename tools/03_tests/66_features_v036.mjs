...
ok(migrated.version===9&&migrated.account.levelXpBase===987&&migrated.pet.feedXp===77,'custom growth values were overwritten');
...
ok(leisureUi.includes('fish-track')&&leisureUi.includes('market-chart')&&leisureUi.includes('完全模拟')&&html.includes('leisure-ui.js?v=0.4.5&build=045'),'fishing/market client UI missing');
...
try{
  const health=await wait();ok(health.version==='0.4.5','server version mismatch');
  ...
  // v0.4.5: the market has a server-side short cooldown; legitimate back-to-back trades must respect it.
  ...
  console.log('[OK] v0.4.5 poker boss / formal blackjack / server-authoritative fishing / simulated market / patch-merge config / accessory toggle');
