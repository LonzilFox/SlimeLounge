...
ok(html.includes('startup-ui.js?v=0.4.5&build=045')&&html.includes('app.js?v=0.4.5&build=045')&&!html.includes('onload="this.media='), 'hotfix asset revision / CSP-safe startup loader missing');
...
try{
  ...
  const a=await fetch(`http://127.0.0.1:${port}/app.js?v=0.4.5&build=045`,{headers:{'accept-encoding':'gzip'}});
  ...
  const b=await fetch(`http://127.0.0.1:${port}/app.js?v=0.4.5&build=045`,{headers:{'if-none-match':etag}});ok(b.status===304,'etag did not return 304');
