...
ok(html.includes('ui-enhancements.js?v=0.4.5&build=045')&&acc.includes('accessoryPalette')&&acc.includes('accessory-svg')&&!acc.includes('accessory-base-layer'),'single-SVG accessory renderer not loaded');
ok(prog.includes('ACCESSORY_ORIGINAL_COLORS')&&prog.includes("c.toLowerCase()==='#ffffff'")&&prog.includes('version:9'),'accessory original-color migration missing');
...
ok(migrated.version===9&&migrated.shop.accessories.find(x=>x.id==='acc_bow')?.color==='#ff7fa2','white accessory migration did not recover bow original color');
...
console.log('[OK] v0.4.5 newest-chat paging / full admin expansion / market UI / single-SVG accessory recoloring+order / pet mood penalties');
