...
ok(prog.includes('ACCESSORY_ORIGINAL_COLORS')&&prog.includes("c.toLowerCase()==='#ffffff'")&&prog.includes('version:10'),'accessory original-color migration missing');
ok(read('public/accessories/star.svg').includes('viewBox="0 0 24 24"')&&read('public/accessories/star.svg').includes('shape-rendering="crispEdges"'),'star asset is not the v0.4.6 pixel redesign');
...
ok(migrated.version===10&&migrated.shop.accessories.find(x=>x.id==='acc_bow')?.color==='#ff7fa2','white accessory migration did not recover bow original color');
