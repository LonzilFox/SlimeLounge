...
const regression=fs.readFileSync(path.join(root,'tools/03_tests/50_regression_project.mjs'),'utf8');
...
ok(leisure.includes('heartbeatMs:650')&&leisure.includes('version:7'),'fishing request-frequency migration missing');
...
ok(leisure.includes('s.fairPrice=Number(s.price)')&&leisure.includes('gapToFair=clamp(Math.log')&&leisure.includes('fairAlpha=1-Math.pow(1-.0007,scale)')&&!leisure.includes('mean=-.0015*Math.log(Math.max(.01,Number(s.price))/Math.max(.01,a.basePrice))'),'market still mean-reverts directly to initial basePrice');
...
ok(regression.includes("readFileSync(path.join(root,'package.json')")&&!regression.includes("health.version!=='0.4.6'"),'legacy regression still hardcodes release version');
ok(runner.includes('73_feature_network_fishing_tetris.mjs')&&runner.includes('74_feature_fishing_margin_ui.mjs'),'v0.4.3/v0.4.6 tests missing from npm run check');
...
try{
 ...
 console.log('[OK] v0.4.6 fish lock / diagnostics / pet penalty / responsive UI / limit+margin trading');
