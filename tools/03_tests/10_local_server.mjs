...
try{
  ...
  if(health.version!=='0.3.4'||health.port!==port)throw Error('health mismatch');
  ...
  const badAudio=await fetch(`http://127.0.0.1:${port}/api/music/audio?id=bad`);if(badAudio.status!==400)throw Error('music audio proxy input validation failed');
  const badProbe=await fetch(`http://127.0.0.1:${port}/api/music/probe?id=bad`);if(badProbe.status!==400)throw Error('music audio probe input validation failed');
  ...
  const rankAfterAi=await (await fetch(`http://127.0.0.1:${port}/api/leaderboards`)).json();
